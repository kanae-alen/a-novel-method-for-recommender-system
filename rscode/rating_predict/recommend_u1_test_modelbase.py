...
def error_function(R, mu, bu, bi, pu, qi, lambda_new):
    ...
    regularization = (lambda_new * (np.sum(np.square(bu)) + np.sum(np.square(bi)) + np.sum(np.square(pu)) + np.sum(np.square(qi)))) / 2
...
def msgd(R, epochs, lambda_new, gamma, batch_size):
    ...
    for epoch in range(epochs):
        ...
        for batch_base in range(batch_count):
            ...
            # 将刚才获取的数据的u,i索引删去重复的，并按照从小到大的顺序排列
            u_index_list = list(set(u_index_list))
            i_index_list = list(set(i_index_list))

            # 遍历u,将剩余的参数更新工作完成
            for u in u_index_list:
                delta_bu[u] = delta_bu[u] - gamma *  lambda_new * bu[u]
                delta_pu[u ,:] = delta_pu[u ,:] - gamma *  lambda_new * pu[u ,:]

            # 遍历i,将剩余的参数更新工作完成
            for i in i_index_list:
                delta_bi[i] = delta_bi[i] - gamma *  lambda_new * bi[i]
                delta_qi[i ,:] = delta_qi[i ,:] - gamma *  lambda_new * qi[i ,:]
